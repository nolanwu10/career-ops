...
const REPO_ROOT = existsSync(join(ROOT, '.git')) ? ROOT : dirname(ROOT);
const LOCAL_PREFIX = relative(REPO_ROOT, ROOT).replaceAll('\\', '/');
...
// In the monorepo, this relocation-aware updater is the compatibility shim.
// Do not replace it with the upstream root-layout copy during an update.
const UPDATE_PATHS = LOCAL_PREFIX
  ? SYSTEM_PATHS.filter(path => path !== 'update-system.mjs')
  : SYSTEM_PATHS;
...
function git(...args) {
  return execFileSync('git', args, { cwd: REPO_ROOT, encoding: 'utf-8', timeout: 30000 }).trim();
}

function localPathspec(path) {
  const clean = path.replace(/\/$/, '');
  return LOCAL_PREFIX ? `${LOCAL_PREFIX}/${clean}` : clean;
}

function restoreRemotePath(ref, path) {
  const clean = path.replace(/\/$/, '');
  const files = git('ls-tree', '-r', '--name-only', ref, '--', clean)
    .split('\n')
    .filter(Boolean);
  if (files.length === 0) throw new Error(`Path not found in ${ref}: ${clean}`);

  for (const remoteFile of files) {
    const destination = join(ROOT, remoteFile);
    mkdirSync(dirname(destination), { recursive: true });
    const contents = execFileSync('git', ['show', `${ref}:${remoteFile}`], {
      cwd: REPO_ROOT,
      encoding: null,
      timeout: 30000,
    });
...
function gitStatusEntries() {
  ...
  return status.split('\n')
    ...
    .filter(entry => !LOCAL_PREFIX || entry.path.startsWith(`${LOCAL_PREFIX}/`))
    .map(entry => ({
      ...entry,
      path: LOCAL_PREFIX ? entry.path.slice(LOCAL_PREFIX.length + 1) : entry.path,
    }));
}

function revertPaths(paths) {
  if (paths.length === 0) return;
  git('checkout', '--', ...paths.map(localPathspec));
}

function addPaths(paths) {
  if (paths.length === 0) return;
  git('add', '--', ...paths.map(localPathspec));
}
...
async function apply() {
  ...
  try {
    ...
    for (const path of BOOTSTRAP_PATHS) {
      if (UPDATE_PATHS.includes(path)) continue; // already in main loop
      try {
        restoreRemotePath('FETCH_HEAD', path);
        updated.push(path);
      } catch {
    ...
    try {
      for (const entry of gitStatusEntries()) {
        ...
        if (UPDATE_PATHS.includes(file)) continue;
...
function rollback() {
  ...
  try {
    ...
    for (const path of UPDATE_PATHS) {
      try {
        git('checkout', latest, '--', localPathspec(path));
        restored.push(path);
      } catch (err) {
        const pathspec = path.endsWith('/') ? path.slice(0, -1) : path;
        let existedInBackup = true;
        try {
          git('cat-file', '-e', `${latest}:${localPathspec(pathspec)}`);
        } catch {
        ...
        git('rm', '-r', '-f', '--ignore-unmatch', '--', localPathspec(pathspec));
        try {
