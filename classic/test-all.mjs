...
const desktopJsFiles = [
  'desktop-app/src/main.js',
  'desktop-app/src/preload.js',
  'desktop-app/src/renderer/renderer.js',
].filter(fileExists);
for (const f of desktopJsFiles) {
  const result = run(NODE, ['--check', f]);
  if (result !== null) {
    pass(`${f} syntax OK`);
  } else {
...
if (
  ...
  scanScript.includes('fetchSearchQuery') &&
  scanScript.includes('Portal searches run:')
) {
  pass('scan.mjs executes configured broad portal searches');
} else {
  fail('scan.mjs does not execute configured broad portal searches');
}

if (
  scanScript.includes('--verify-limit=') &&
  scanScript.includes('Verification deferred:') &&
  scanScript.includes('directOffers')
) {
  pass('scan.mjs bounds broad-search verification and trusts direct providers');
} else {
  fail('scan.mjs does not bound broad-search verification');
}

try {
  const { parseSearchHtml } = await import(pathToFileURL(join(ROOT, 'scan.mjs')).href);
  const parsed = parseSearchHtml(
    '<div class="compTitle options-toggle"><a href="https://r.search.yahoo.com/x/RU=https%3a%2f%2fjobs.example.com%2f123%3fx%3d1%26y%3d2/RK=2/RS=x">' +
    '<h3 class="title"><span>Data Science Intern at Example Co</span></h3></a></div>'
  );
  if (
    parsed.length === 1 &&
    parsed[0].title === 'Data Science Intern at Example Co' &&
    parsed[0].url === 'https://jobs.example.com/123?x=1&y=2'
  ) {
    pass('broad portal search parser extracts and decodes results');
  } else {
    fail('broad portal search parser returned unexpected output');
  }
} catch (err) {
  fail(`broad portal search parser test failed: ${err.message}`);
}
