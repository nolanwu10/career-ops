...
function normalizeStatus(raw) {
  ...
  if (/^(oa|online assessment|assessment)$/i.test(s)) return { status: 'Online Assessment' };
  ...
  const canonical = [
    'Evaluated', 'Applied', 'Online Assessment', 'Responded', 'Interview',
  ...
  if (['oa', 'assessment', 'online assessment'].includes(lower)) return { status: 'Online Assessment' };
