...
var (
	...
	// "City on-site" / "City in-office" / "City onsite" — work-mode keyword anchors
	// the preceding title-case word(s) as a city, catching international cities and
	// US cities written without a state code (e.g. "Miami on-site", "NYC on-site").
	reCityWorkMode = regexp.MustCompile(`\b([A-Z][A-Za-z.'-]+(?: [A-Z][A-Za-z.'-]+){0,2}) (?:on-site|in-office|onsite)\b`)
...
func deriveNoteFields(app *model.CareerApplication) {
	...
	// Location: prefer "City, ST" (US city+state); fall back to "City on-site /
	// in-office" (work-mode keyword anchors the city, catches international cities
	// and US cities written without a state code); then try the role title.
	if m := reCityState.FindStringSubmatch(app.Notes); m != nil {
		app.Location = m[1] + ", " + m[2]
	} else if m := reCityWorkMode.FindStringSubmatch(app.Notes); m != nil {
		app.Location = m[1]
	} else if m := reCityState.FindStringSubmatch(app.Role); m != nil {
		app.Location = m[1] + ", " + m[2]
	} else if m := reCityWorkMode.FindStringSubmatch(app.Role); m != nil {
		app.Location = m[1]
	}
