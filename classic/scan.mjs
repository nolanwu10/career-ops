 ...
 *   node scan.mjs --no-broad-search # skip configured search_queries
...
const SEARCH_ENDPOINT = 'https://search.yahoo.com/search?p=';
...
function buildTitleFilter(titleFilter) {
  const positive = expandTitleKeywords(titleFilter?.positive || []);
  const negative = expandTitleKeywords(titleFilter?.negative || []);

  return (title) => {
    const lower = normalizeTitleKeyword(title);
...
function expandTitleKeywords(keywords) {
  const expanded = new Set();
  for (const raw of keywords) {
    if (typeof raw !== 'string') continue;
    const keyword = normalizeTitleKeyword(raw);
    if (!keyword) continue;
    expanded.add(keyword);
    if (/\bengineering\b/.test(keyword)) expanded.add(keyword.replace(/\bengineering\b/g, 'engineer'));
    if (/\bengineer\b/.test(keyword)) expanded.add(keyword.replace(/\bengineer\b/g, 'engineering'));
    if (keyword === 'software engineering' || keyword === 'software engineer') expanded.add('swe');
    if (keyword === 'swe') {
      expanded.add('software engineer');
      expanded.add('software engineering');
    }
  }
  return [...expanded];
}
...
function resolveYahooResultUrl(rawUrl) {
  const decoded = decodeXml(rawUrl);
  try {
    const parsed = new URL(decoded);
    if (parsed.hostname === 'r.search.yahoo.com') {
      const encodedTarget = parsed.pathname.match(/\/RU=([^/]+)\/RK=/)?.[1];
...
export function parseSearchHtml(html) {
  const results = [];
  const pattern = /<div class="compTitle[\s\S]*?<a[^>]+href="([^"]+)"[\s\S]*?<h3[^>]*>[\s\S]*?<span[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/h3>[\s\S]*?<\/a>/gi;
  for (const match of String(html || '').matchAll(pattern)) {
    const url = resolveYahooResultUrl(match[1]);
...
function queryDomains(query) {
  return [...new Set(
    [...String(query || '').matchAll(/site:([a-z0-9.-]+)/gi)]
      .map(match => match[1].toLowerCase().replace(/^\.+|\.+$/g, ''))
      .filter(Boolean)
  )];
}

function urlMatchesQueryDomains(url, domains) {
...
function companyFromSearchResult(title, url) {
  const atMatch = title.match(/\s+(?:@|at)\s+([^|—–]+)(?:\s*[|—–]|$)/i);
  if (atMatch) return atMatch[1].trim();
  const pipeParts = title.split(/\s+[|—–]\s+/).map(part => part.trim()).filter(Boolean);
  if (pipeParts.length > 1 && !/linkedin|ashby|greenhouse|lever|jobs?/i.test(pipeParts[1])) {
    return pipeParts[1];
  }
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split('/').filter(Boolean);
    if (/jobs\.ashbyhq\.com|jobs\.lever\.co|job-boards(?:\.eu)?\.greenhouse\.io/.test(parsed.hostname)) {
      return segments[0] || parsed.hostname;
    }
...
function titleFromSearchResult(title) {
  return title
    .replace(/\s+(?:@|at)\s+[^|—–]+(?:\s*[|—–].*)?$/i, '')
    .replace(/\s+[|—–]\s+(?:LinkedIn|Ashby|Greenhouse|Lever|Jobs?).*$/i, '')
    .trim();
}

export async function fetchSearchQuery(queryEntry, ctx) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      html = await ctx.fetchText(`${SEARCH_ENDPOINT}${encodeURIComponent(query)}`, {
        timeoutMs: 20_000,
        headers: {
  ...
  return parseSearchHtml(html)
    ...
    .filter(result => urlMatchesQueryDomains(result.url, domains))
    .map(result => ({
      title: titleFromSearchResult(result.title),
      url: result.url,
      company: companyFromSearchResult(result.title, result.url),
      location: '',
      source: queryEntry.name || 'portal-search',
    }))
...
async function main() {
  ...
  const broadSearch = !args.includes('--no-broad-search');
  const verifyLimitArg = args.find(arg => arg.startsWith('--verify-limit='));
  const parsedVerifyLimit = verifyLimitArg ? Number(verifyLimitArg.split('=')[1]) : Infinity;
  const verifyLimit = Number.isFinite(parsedVerifyLimit) && parsedVerifyLimit >= 0
    ? Math.floor(parsedVerifyLimit)
    : Infinity;
  ...
  const searchQueries = broadSearch
    ? (config.search_queries || []).filter(query => query && query.enabled !== false && query.query)
    : [];
  ...
  if (searchQueries.length > 0) {
    console.log(`\nSearching ${searchQueries.length} configured portal queries...`);
    const searchTasks = searchQueries.map(queryEntry => async () => {
      try {
        const jobs = await fetchSearchQuery(queryEntry, makeHttpCtx());
        searchQueriesRun++;
        searchResultsFound += jobs.length;
        totalFound += jobs.length;
        for (const job of jobs) {
          if (!titleFilter(job.title)) {
            totalFilteredTitle++;
        ...
        errors.push({ company: queryEntry.name || 'portal-search', error: err.message });
      } finally {
  ...
  let droppedOffers = [];
  let invalidOffers = [];
  let deferredVerification = [];
  if (verify && newOffers.length > 0) {
    const directOffers = newOffers.filter(offer => /^(?:local-parser|[a-z0-9-]+-api)$/i.test(offer.source));
    const searchOffers = newOffers.filter(offer => !directOffers.includes(offer));
    const offersToVerify = searchOffers.slice(0, verifyLimit);
    deferredVerification = searchOffers.slice(verifyLimit);
    verifiedOffers = [...directOffers];
    if (offersToVerify.length > 0) {
      console.log(`\nVerifying liveness of ${offersToVerify.length} broad-search offer(s) with Playwright (sequential)...`);
      const result = await verifyOffers(offersToVerify, { headedFallback, throttleBaseMs });
      verifiedOffers.push(...result.verified);
      expiredOffers = result.expired;
      droppedOffers = result.dropped;
      invalidOffers = result.invalid;
    }
    if (deferredVerification.length > 0) {
      console.log(`Deferred ${deferredVerification.length} broad-search result(s) to later scans (verification limit: ${verifyLimit}).`);
    }
  }
  ...
  console.log(`Portal searches run:   ${searchQueriesRun}/${searchQueries.length}`);
  console.log(`Portal search results: ${searchResultsFound}`);
  ...
  if (verify) {
    ...
    console.log(`Verification deferred: ${deferredVerification.length}`);
  }
